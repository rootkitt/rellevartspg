...
// 请求应用列表
// 参数->rewarded @YES 有能量点模式  @NO 无能量点模式
//
// 详解:
//      使用该方法请求的应用列表为默认的Web页面，你需要通过使用showOffers或showOffers:来显示界面
// 补充:
//      1.有能量点模式: 应用本身采用能量点激励下载模式
//      2.无能量点模式: 应用本身无能量点激励模式，适应于应用交叉推广
//
// 回调:
...
//
// 详解:
//      当用户安装完成应用并执行了相关操作后，后台将会保存相应的安装记录和所获得的能量点情况。
//      通过该接口，你可以获取用户所赚取的能量点
// 补充:
...
//
// 详解:
//      该方法和requestEarnedPoints方法作用相同，都是查询用户获取的能量点情况。
