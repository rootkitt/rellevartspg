...
- (void) viewDidLoad
{
    ...
    UIButtonLarge* sGlobalViewButton = [UIButtonLarge buttonWithType: UIButtonTypeCustom];
    ...
    sGlobalViewButton.mMarginInsets = UIEdgeInsetsMake(15, 10, 0, 10);
...
{
    CLLocation* sAdjustedRegularLocation = userLocation.location;
    NSLog(@"--------------mapView:didUpdateUserLocation--------------------");

    
    [self getNewCurrentLocation:sAdjustedRegularLocation];
}
...
{
    [self getNewCurrentLocation:aNewLocation];
#ifdef DEBUG
 NSLog(@"--------------locationSource:withNewLocation--------------------");
#endif
}


- (void) getNewCurrentLocation:(CLLocation*)aLocation
{
    CLLocation* sAdjustedRegularLocation = aLocation;
